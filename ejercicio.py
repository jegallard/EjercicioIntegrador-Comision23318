...
class Persona():
    def __init__(self, nombre, edad, dni):
        try:
            if type(nombre) == str and  nombre.isalpha():
                ...
                raise ValueError("Error: el atributo 'nombre' solo puede ser alfabético")
            
            if type(edad) == int and edad > 0:
                self.__edad = edad
            elif type(edad) == int and edad < 0:
                raise ValueError("Error: el atributo 'edad' no puede ser menor a 0")
            elif type(edad) == float or type(edad)== str:
                raise ValueError("Error: el atributo 'edad' solo puede entero")
            
            if type(dni) == int and dni > 0 and len(str(dni)) > 6 and len(str(dni)) < 9:
                self.__dni = dni
            else:
                raise ValueError("Error: DNI inválido. Ingrese solo números sin signos de puntuación (Ejemplo: 12345678)")            
    ...
    def nombre(self,nombre):
        if nombre.isalpha():
            ...
            print("El parametro 'nombre' no puede ser numérico")
    ...
    def __init__(self, nombre, edad, dni, cantidad=0):
        ...
        try:
            if type(cantidad) in (float, int):
                ...
                raise ValueError("El atributo 'cantidad' debe ser numérico")
        except ValueError as e:
            print(e)
    ...
    def ingresarCantidad(self, numero):
        try:
            if type(numero) in (int, float): 
                ...
                raise ValueError("ERROR: Metodo '.ingresarCantidad()' debe ser numérico")
        except ValueError as e:
            print(e)
    ...
    def retirarCantidad(self, numero):
        try:
            if type(numero) in (int, float): 
                ...
                raise ValueError("ERROR: Metodo '.retirarCantidad()' debe ser numérico")
        except ValueError as e:
            print(e)

cuenta1 = Cuenta("Jorge",38,12345678)
cuenta1.mostrar()
